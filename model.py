...
for index, row in df.iterrows():
    ...
    df.loc[index] = row     # Changed df.ix[index] to df.loc[index], .ix is deprecated, was getting errors - Sam
...
for index, row in df.iterrows():
    ...
    visitor_team = row["Visitor"]
    indexer = 0
    for team in standing['Unnamed: 1']:                     # Replaced
        if home_team == team:                               # home_rank = standing[standing["School"] == home_team]["Rk"].values[0]
            home_rank = standing['Unnamed: 0'][indexer]     # and visitor_rank = standing[standing["School"] == visitor_team]["Rk"].values[0]
        elif visitor_team == team:                          # with this for loop
            visitor_rank = standing['Unnamed: 0'][indexer]  # - Sam
        indexer += 1                                        #
...
X_small = df[['Home Ranks Higher', 'Home Win Streak']]      # was 'Home Team Ranks Higher', changed to correct index 'Home Ranks Higher'  - Sam
